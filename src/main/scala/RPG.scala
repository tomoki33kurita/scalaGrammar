...
object RPG extends App {
  ...
  while (!monsters.isEmpty) {
    ...
    if(!hero.isAlive) {
      ...
      System.exit(0)
    } else if(!monster.isAlive() || monster.isAwayFromHero) { // モンスターがいないかどうか
      if(!monster.isAwayFromHero) { // 倒した場合
        println("モンスターは倒れた。そしてあなたは、モンスターの武器をゲットした。")
...
// abstractクラス自体をインスタンス化することはできない。継承で利用することが前提
...
class Hero(_hitPoint: Int, _attackDamage: Int) extends Creature(_hitPoint, _attackDamage) {
  ...
  //このoverrideは、JavaのObjectクラスを暗黙的に継承・参照している...故にJavaのObjectクラスのtoStringメソッドをoverrideしている
